...
module.exports = Syifa = async (Syifa, mek) => {
		try {
...
//━━━━━━━━━━━━━━━[ Terakhir ]━━━━━━━━━━━━━━━\\
			const botNumber = Syifa.user.jid
			const ownerNumber = [`${nomorowner}@s.whatsapp.net`] 
			const isGroup = from.endsWith('@g.us')
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			pushname = Syifa.contacts[sender] != undefined ? Syifa.contacts[sender].vname || Syifa.contacts[sender].notify : undefined
			const groupMetadata = isGroup ? await Syifa.groupMetadata(from) : ''
...
//━━━━━━━━━━━━━━━[ HARI HARI ]━━━━━━━━━━━━━━━\\
        ...
        const isUrl = (url) => {
			...
			const reply = (teks) => {
			Syifa.sendMessage(from, teks, text, { thumbnail: gember, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} WIB - ${week}`,body:"New Base SyifaBot",previewType:"PHOTO",thumbnail:gember,sourceUrl:`https://wa.me/p/6349482305092740/6285697662826`}}})
		}
		const reply2 = (teks) => {
Syifa.sendMessage(from, teks, text, {quoted: mek, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})
}
			const sendMess = (hehe, teks) => {
				Syifa.sendMessage(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? Syifa.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : Syifa.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
			if (budy.includes("https://chat.whatsapp.com/")) {
				...
				setTimeout(() => {
				Syifa.groupRemove(from, [kic]).catch((e) => { reply(`BOT HARUS JADI ADMIN`) })
				}, 0)
			   }
//━━━━━━━━━━━━━━━[WAKTU]━━━━━━━━━━━━━━━\\
			            var ase = new Date();
...
//━━━━━━━━━━━━━━━[ FAKE FAKEAN ]━━━━━━━━━━━━━━━\\
           ...
           const fakedoc = {key: {fromMe: false, participant: `${nomorowner}@s.whatsapp.net`, ...(from ? {remoteJid: "status@broadcast" } : {}) }, message: {documentMessage: {mimetype: 'application/octet-stream', title: `${ucapanFakereply}`, pageCount: 0, fileName: `SyifaBot`, jpegThumbnail: fs.readFileSync(`./media/gambar/thumb.jpg`)}}}
...
const ftrolMENU = {key : {fromMe:false,
	participant : '0@s.whatsapp.net'},
       message: { 
              orderMessage: {
                            ...
                            contextInfo: {"forwardingScore":999,"isForwarded":true}, sendEphemeral: true}}}
			const fakeitem = (teks) => {
           Syifa.sendMessage(from, teks, text, {
                   quoted: {
                           key:{
                 	       fromMe:false, 
                           participant:`0@s.whatsapp.net`, ...(from ? {
	...
	Syifa.on('CB:action,,battery', json => {
    ...
    Syifa.sendMessage(from, buttonMessagesI, MessageType.buttonsMessage, {quoted: mek})
  }
    const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    Syifa.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
      ...
      kma = gam1;
      mhan = await Syifa.prepareMessage(from, kma, video);
      ...
      Syifa.sendMessage(id, buttonMessages, MessageType.buttonsMessage, {quoted: mek})
    };
        
        const sendButloc = async(id, text1, desc1, gam1, but = [], options = {}) => {
               let kma = gam1
               Syifa.sendMessage(id, {"contentText": text1,
               "footerText": desc1, 
               "buttons": but,
               "headerType": "LOCATION",
                       "locationMessage": {
      ...
      Syifa.sendMessage(id, buttonMessage, MessageType.buttonsMessage, {quoted: mek})
    };
...
return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net')
			...
			let authorname = Syifa.contacts[from] != undefined ? Syifa.contacts[from].vname || Syifa.contacts[from].notify : undefined	
			if (authorname != undefined) { } else { authorname = groupName }	
			
			function addMetadata(packname, author) {	
				if (!packname) packname = 'Bot'; if (!author) author = 'By SyifaBot';	
...
async function sendButLocation(id, text1, desc1, gam1, but = [], options = {}) {
            ...
            return Syifa.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
        }       
Syifa.updatePresence(from, Presence.composing)
//━━━━━━━━━━━━━━━[ PUBLIC ]━━━━━━━━━━━━━━━\\
if (!publik) {
...
const froxx = {
	 ...
	 message: { 
		"extendedTextMessage": {
                 "text": `New Base SyifaBot`,
                 "title": `New Base Syifa Bot`,
...
//━━━━━━━━━━━━━━━[ BUAT BIO BOT ]━━━━━━━━━━━━━━━\\
runi = process.uptime() 
           Syifa.setStatus(`Syifa Bot Aktif Selama ${kyun(runi)} `).catch((_)=>_);
...
//━━━━━━━━━━━━━━━[ TEKS BUFFER ]━━━━━━━━━━━━━━━\\
const fakeText = (teks) => {
Syifa.sendMessage(from, teks, text, {quoted: froxx})
}
//━━━━━━━━━━━━━━━[ SETTING ]━━━━━━━━━━━━━━━\\
...
┃⬡${prefix}bear {teks}
└──────────────`
sendButImage(from, anu,`𝑺𝒆𝒍𝒂𝒎𝒂𝒕 𝑴𝒆𝒏𝒄𝒐𝑩𝒂 𝑺𝒚𝒊𝒇𝒂 𝑩𝒐𝒕 , 𝑭𝒊𝒕𝒖𝒓 𝑺𝒆𝒅𝒂𝒏𝒈 𝒅𝒂𝒍𝒂𝒎 𝑻𝒂𝒉𝒂𝒑 𝑷𝒆𝒏𝒈𝒆𝒎𝒃𝒂𝒏𝒈𝒂𝒏 𝑱𝒂𝒅𝒊 𝑱𝒊𝒌𝒂 𝑭𝒊𝒕𝒖𝒓 𝑬𝒓𝒐𝒓 𝑭𝒊𝒙 𝑺𝒆𝒏𝒅𝒊𝒓𝒊 𝒀𝒂 𝑭𝒓𝒆𝒆 𝑨𝒑𝒊𝒌𝒆𝒚 𝑪𝒓𝒆𝒂𝒕𝒐𝒓 𝑱𝒂𝒏𝒈𝒂𝒏 𝑫𝒊 𝑼𝒃𝒂𝒉 @𝑪𝒐𝒑𝒚𝑹𝒆𝒄 𝑩𝒚 𝑫𝒉𝒆𝒏𝒙𝒔 𝑪𝒐𝒅𝒆 𝑺𝒆𝒍𝒂𝒎𝒂𝒕 𝒎𝒆𝒏𝒄𝒐𝒃𝒂\n${Tanggal}`, thumb, [
...
Syifa.sendMessage(from, sound, MessageType.audio, {quoted: froxx, mimetype: 'audio/mp4', ptt:true})
break
case 'sc':
case 'scrip':
if (isBanned) return reply(mess.banned)
reply2(`JANGAN LUPA SUBSCRIBE YouTube : https://youtu.be/cgaK-nGFvT8`)
break
//══════════[ ASUPAN MENU ]════════════════════════════//
             case 'asupangeayubi':
             if (isBanned) return reply(mess.banned)
             ...
             videoMsg = (await Syifa.prepareMessageMedia(geayubi, "videoMessage", { thumbnail: geayubi, })).videoMessage
              buttonsMessage = {footerText:'KLIK NEXT UNTUK SELANJUTNYA', videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Syifa.prepareMessageFromContent(from,{buttonsMessage},{quoted: fhidetag})
              Syifa.relayWAMessage(prep)
              break
             case 'asupanaura':
             if (isBanned) return reply(mess.banned)
             if (!isPremium) return reply(mess.only.premium)
             ...
             buttons = [{buttonId: `${prefix}caca`,buttonText:{displayText: `➡️ NEXT`},type:1}]
             videoMsg = (await Syifa.prepareMessageMedia(naura, "videoMessage", { thumbnail: naura, })).videoMessage
              buttonsMessage = {footerText:'KLIK NEXT UNTUK SELANJUTNYA', videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Syifa.prepareMessageFromContent(from,{buttonsMessage},{quoted: fhidetag})
              Syifa.relayWAMessage(prep)
            ...
            const bunga = await getBuffer(`https://api-alphabot.herokuapp.com/api/asupan/bunga?apikey=${AlphaBot}`)
             buttons = [{buttonId: `${prefix}caca`,buttonText:{displayText: `➡️ NEXT`},type:1}]
              videoMsg = (await Syifa.prepareMessageMedia(bunga, "videoMessage", { thumbnail: bunga, })).videoMessage
              buttonsMessage = {footerText:'KLIK NEXT UNTUK SELANJUTNYA', videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Syifa.prepareMessageFromContent(from,{buttonsMessage},{quoted: fhidetag})
              Syifa.relayWAMessage(prep)
            ...
            const ayu = await getBuffer(`https://api-alphabot.herokuapp.com/api/asupan/ayu?apikey=${AlphaBot}`)
             buttons = [{buttonId: `${prefix}caca`,buttonText:{displayText: `➡️ NEXT`},type:1}]
             videoMsg = (await Syifa.prepareMessageMedia(ayu, "videoMessage", { thumbnail: ayu, })).videoMessage
              buttonsMessage = {footerText:'KLIK NEXT UNTUK SELANJUTNYA', videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Syifa.prepareMessageFromContent(from,{buttonsMessage},{quoted: fhidetag})
              Syifa.relayWAMessage(prep)
             ...
             buttons = [{buttonId: `${prefix}caca`,buttonText:{displayText: `➡️ NEXT`},type:1}]
              videoMsg = (await Syifa.prepareMessageMedia(caca, "videoMessage", { thumbnail: caca, })).videoMessage
              buttonsMessage = {footerText:'KLIK NEXT UNTUK SELANJUTNYA', videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Syifa.prepareMessageFromContent(from,{buttonsMessage},{quoted: fhidetag})
              Syifa.relayWAMessage(prep)
...
//══════════[ANIME MENU]════════════════════════════//
            ...
            if (isBanned) return reply(mess.banned)
             ...
             cewe = await getBuffer(anu.result.female)
              ...
              Syifa.sendMessage(from, cowo, image, {quoted: froxx, caption: 'Nih Versi Cowo Nya ^_^' })
              Syifa.sendMessage(from, cewe, image, {quoted: froxx, caption: 'Nih Versi Cewe Nya ^_^' })
              break
                case 'sao':
                if (isBanned) return reply(mess.banned)
				Syifa.updatePresence(from, Presence.composing) 
                 ...
                 Syifa.sendMessage(from, swordartonline, image, {quoted: froxx, caption: 'swort art online\nMEZ RazoR'})
				 break
				case 'hsdxd':
				if (isBanned) return reply(mess.banned)
				 Syifa.updatePresence(from, Presence.composing) 
                 ...
                 Syifa.sendMessage(from, highschooldxd, image, {quoted: froxx, caption: 'NIH BANG '})
				break
				 case 'lovelive':
				if (isBanned) return reply(mess.banned)
				 Syifa.updatePresence(from, Presence.composing) 
                 ...
                 Syifa.sendMessage(from, lovelive, image, {quoted: froxx, caption: 'NIH BANG '})
				break
                    case 'uniform':
					case 'sfwneko':
					case 'cuckold':
					...
					Syifa.sendMessage(from, qute6, image, { quoted: froxx, caption: ':)' })
					break
             case 'waifu':
             case 'loli':
            case 'husbu':
            ...
            if (isBanned) return reply(mess.banned)
              ...
              imageMsg = ( await Syifa.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'Menuju Tak Terbatas', imageMessage: imageMsg,
              contentText:`Creator SyifaCode`,buttons,headerType:4}
              prep = await Syifa.prepareMessageFromContent(from,{buttonsMessage},{quoted: froxx})
              Syifa.relayWAMessage(prep)
...
//══════════[ ANIMASI HEWAN ]════════════════════════════//
                   ...
                   Syifa.sendMessage(from, anu1, image, {caption: `nih kak mirip kamu`, quoted: froxx})
                   break
                   case 'dog':  
                   if (isBanned) return reply(mess.banned)
...
// Random Image //
                ...
                reply(mess.wait)
                    getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=${Lolhuman}`).then((gambar) => {
                        Syifa.sendMessage(from, gambar, image, { quoted: froxx })
                    })
                    break
                case 'chiisaihentai':
                case 'trap':
...
//══════════[ tts Gunakan Kode Bahasa ]════════════════════════════//
                     case 'bahasa':
                     if (isBanned) return reply(mess.banned)
                    Syifa.sendMessage(from, bahasa(), text, { quoted:froxx })
                    break 
                    case 'kodenegara':
                    if (isBanned) return reply(mess.banned)
					Syifa.sendMessage(from, negara(), text)
					break
                    case 'tts':
                    if (isBanned) return reply(mess.banned)
				    if (args.length < 1) return Syifa.sendMessage(from, 'Diperlukan kode bahasa kak!!', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return Syifa.sendMessage(from, 'Mana teks yang mau di jadiin suara? suara setan kah?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 500
...
break
				...
				Syifa.sendMessage(from, teks.trim(), extendedText, {quoted: froxx, contextInfo: {"mentionedJid": prem}})
				break
//══════════[ BAN MENU ]════════════════════════════//
case 'ban':
if (!isOwner) return reply(mess.only.ownerB)
			...
			break
					...
					if (isBanned) return reply(mess.banned)
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							const media = await Syifa.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											Syifa.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: froxx})
											fs.unlinkSync(media)	
...
Syifa.sendMessage(from, mp4, video)
break
case 'mp3':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
Syifa.sendMessage(from, mp3, audio)
break
case 'ytmp3':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
Syifa.sendMessage(from, thu, image, { quoted: mek, caption: ini_txt })
res = await getBuffer(get.url_audio)
Syifa.sendMessage(from, res, audio)
break
case 'ytmp4':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
Syifa.sendMessage(from, res, video)
break
case 'tiktok':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
reply(mess.wait)
              ...
              Syifa.sendMessage(from, ini_video, video, { quoted: froxx })
              break
          case 'buttons4': 
          if (isBanned) return reply(mess.banned)
           reply(mess.wait)
              ...
              Syifa.sendMessage(from, data, audio, { quoted: froxx })
              break
              case 'buttons1':
              await axios.get(`https://api.zeks.xyz/api/ytplaymp3/2?apikey=${ZeksKey}&q=${q}`)
		     .then(res => {
			  Syifa.sendMessage(from, { url: res.data.result.link }, 'audioMessage', { mimetype: 'audio/mp4', quoted: froxx, contextInfo: { externalAdReply: { title: res.data.result.title, mediaType: 2, thumbnailUrl: res.data.result.thumb, mediaUrl: res.data.result.source }}})
})
              break
case 'lirik':
if (isBanned) return reply(mess.banned)
...
case 'bc': 
					...
					anu = await Syifa.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await Syifa.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							Syifa.sendMessage(_.jid, bc, image, {quoted:mek,caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
             Syifa.sendMessage(_.jid, 
			{"contentText": `*「 BROADCAST ${namabot} 」*\n\n${body.slice(4)}`,
			"footerText": 'by Syifa Bot',
			"buttons": [
        ...
        if (isBanned) return reply(mess.banned)
             ...
             if (isMedia && !mek.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             bc = await Syifa.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             	tes = `${body.slice(4)}`
             	Syifa.sendMessage(_.jid, bc, { contentText: `${tes}`, footerText: `_*${namabot} Broadcast*_`, buttons: [{buttonId: `${prefix}menu`,buttonText:{displayText: '𝐌𝐞𝐧𝐮'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff10, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
             	textt = `${body.slice(4)}`
             Syifa.sendMessage(_.jid, { contentText: `${textt}`, footerText: `_*${namabot} Broadcast*_`, buttons: [{buttonId: `${prefix}menu`,buttonText:{displayText: '𝐌𝐞𝐧𝐮'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff10, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')}
             reply('Suksess broadcast')}
             break
//══════════[ WELCOME MENU ]════════════════════════════//
case 'intro':
...
└ 𝐉𝐞𝐧𝐢𝐬 𝐤𝐞𝐥𝐚𝐦𝐢𝐧:
`
Syifa.sendMessage(from, intro, text, {quoted: ftrolMENU, contextInfo: {"forwardingScore":999,"isForwarded":true}, sendEphemeral: true })
break
//══════════[ SETIFIKAT MENU]════════════════════════════//
case 'serti1':
case 'serti2':
...
Syifa.sendMessage(from, buff, image, { quoted: froxx, caption: 'Nih Bro Hasil nya' })
break
//══════════[ BERMAIN MENU ]════════════════════════════//
case 'nickepep':
if (isBanned) return reply(mess.banned)
...
Syifa.sendMessage(from, buff, image, {quoted: froxx, caption : teks})
break
//══════════[ PEMBATASAN]════════════════════════════//
case 'notif':
if (isBanned) return reply(mess.banned)
...
group = await Syifa.groupMetadata(from);
...
await Syifa.sendMessage(from, options, text)
break
case 'wa.me':
case 'wame':
if (isBanned) return reply(mess.banned)
...
Syifa.sendMessage(from, options, text, { quoted: mek } )
break
...
const vcard = 'BEGIN:VCARD\n'  
            ...
            + 'END:VCARD'  
  Syifa.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: mek})
titid = 'Butuh info tentang apa ya?'
          ...
          {buttonId: `${prefix}menu`, buttonText: { displayText: `𝑴𝑬𝑵𝑼`, }, type: 1, },
                 ...
                 if (isBanned) return reply(mess.banned)
					...
					Syifa.sendMessage(`${nomorowner}@s.whatsapp.net`, options, text, {quoted: mek})
...
if (isBanned) return reply(mess.banned)
      ...
      if (isBanned) return reply(mess.banned)
             ...
             let totalgroup = Syifa.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
        ...
        if (isBanned) return reply(mess.banned)
            ...
            var response = await Syifa.acceptInvite(codeInvite)
            fakeitem('SUKSES')
            } catch {
            fakeitem('LINK ERROR!')
            }
        ...
        if (isBanned) return reply(mess.banned)
             ...
             fak = Syifa.query({ json: ['action', 'invite', link],
             expect200: true })
...
//GROUP MENU
 ...
 if (!isGroup) return reply(`Only group`)
             ...
             let online = [...Object.keys(Syifa.chats.get(ido).presences), Syifa.user.jid]
             Syifa.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
  ...
  if (!isGroup) return reply(mess.only.group)
              ...
              Syifa.groupUpdateSubject(from, q)
  ...
  if (!isGroup) return reply(mess.only.group)
              ...
              Syifa.groupUpdateDescription(from, q)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              break
       case 'setppgrup':
       if (isBanned) return reply(mess.banned)
           if (!isGroup) return reply(mess.only.group)
              ...
              let media = await Syifa.downloadMediaMessage(encmedia)
              Syifa.updateProfilePicture(from, media)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              } else {
		...
		if (!isBotGroupAdmins) return reply(mess.only.Badmin)
		 ...
		 for (let mem of groupMembers) {
		 	...
		 	members_id.push(mem.jid)
		 		  ...
		 		  Syifa.groupDemoteAdmin(from, members_id)
	...
	if (isBanned) return reply(mess.banned)
		 		    	...
		 		    	for (let mem of groupMembers) {
		 		    	 	...
		 		    	 	members_id.push(mem.jid)
		 		    	 	 	   ...
		 		    	 	 	   Syifa.groupMakeAdmin(from, members_id)
...
if (isBanned) return reply(mess.banned)
      if (!mek.key.fromMe && !isGroupAdmins) return reply("Only admin");
        ...
        Syifa.groupSettingChange(from, GroupSettingChange.messageSend, true);
        break; 
      case "opengc":
      if (isBanned) return reply(mess.banned)
  if (!mek.key.fromMe && !isGroupAdmins) return reply("Only admin");
        ...
        Syifa.groupSettingChange(from, GroupSettingChange.messageSend, false);
        break; 
                case 'hidetag':        
   if (isBanned) return reply(mess.banned)                
					if (!isGroup) return reply(mess.only.group)
					...
					var group = await Syifa.groupMetadata(from)
					...
					Syifa.sendMessage(from, value, text, {quoted: fhidetag, contextInfo: { mentionedJid: mem }})
					break;
									case 'tagall':
									if (isBanned) return reply(mess.banned)
if (!isGroup) return reply(mess.only.group)
...
if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						...
						Syifa.groupRemove(from, mentioned)
					} else {
						mentions(`Berhasil Promote @${mentioned[0].split('@')[0]} Sebagai Admin Group!`, mentioned, true)
						Syifa.groupMakeAdmin(from, mentioned)
					}
					break
				case 'demote':
				if (isBanned) return reply(mess.banned)
			if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						...
						Syifa.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'add':
				if (isBanned) return reply(mess.banned)
			...
			if (!isGroup) return reply(mess.only.group)
                ...
                if (isBanned) return reply(mess.banned)
                 if (!isGroup) return reply(mess.only.group)
                    if (!isGroupAdmins) return reply(mess.only.admin)
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    linkgc = await Syifa.groupInviteCode(from)
                    reply('https://chat.whatsapp.com/'+linkgc)
                    break
                case 'leave':
            if (!isGroup) return reply(mess.only.group)
                    if (isGroupAdmins || isOwner) {
                    	Syifa.groupLeave(from)
                    } else {
...
if (isBanned) return reply(mess.banned)
        ...
        break;
                ...
                if (isBanned) return reply(mess.banned)
					...
					if (Number(args[0]) === 1) {
						...
						Syifa.sendMessage(from,`Perhatian kepada seluruh member anti link group aktif apabila anda mengirim link group anda akan di kick dari group`, text)
					} else if (Number(args[0]) === 0) {
        ...
        if (isBanned) return reply(mess.banned)
     if (!isGroup) return reply(mess.only.group)
					Syifa.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
case 'stikernowm': 
				case 'stickernowm':
				case 'snowm':
				if (isBanned) return reply(mess.banned)
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await Syifa.downloadAndSaveMediaMessage(encmedia)
						ran = getRandom('.webp')
						await ffmpeg(`./${media}`)
							.input(media)
							.on('start', function (cmd) {
...
Syifa.sendMessage(from, 'Waalaikumsalam, tuben dahh salam', text, {quoted: mek})
                  }
if (budy.includes(`Bot`)) {
Syifa.sendMessage(from, 'Ada apa?, bot aktif kok', text, {quoted: mek})
                  }
if (budy.includes(`Tes`)) {
Syifa.sendMessage(from, 'Hmmm', text, {quoted: mek})
                  }
}
if (budy.startsWith('x')){
try {
return Syifa.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
